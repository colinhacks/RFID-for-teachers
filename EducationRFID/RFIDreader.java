...
public class RFIDreader implements SerialPortEventListener {
    ...
    public RFIDreader(TeacherWindow gui) {
    	this.gui = gui;
    }
    
    public RFIDreader(StudentClient studentClient) {
		this.gui = gui;
	}
	...
	private void addToBuffer(byte b) {
		...
		if ("0123456789ABCDE".contains(c)) {
			//System.out.println("constructing next entry for buffer");
			...
			try {
				gui.currentID = buffer.take();
				gui.messageBox.setText("Input found. ID = " + String.valueOf(gui.currentID));
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
